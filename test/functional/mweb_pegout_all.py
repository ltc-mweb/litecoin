...
class MWEBPegoutAllTest(BitcoinTestFramework):
    def set_test_params(self):
        ...
        self.num_nodes = 1
        #self.extra_args = [["-mintxfee=0.00000100"]]
    ...
    def run_test(self):
        ...
        total_balance = self.nodes[0].getbalance(avoid_reuse=False)
        self.log.info(total_balance)

        self.log.info("Pegout all coins")
        pegout_amount = (total_balance - Decimal('2.000'))
        self.log.info(pegout_amount)
        ...
        self.log.info("Fee: {}".format(pegout_tx['fee']))
        self.log.info(pegout_tx['decoded']['vkern'])
